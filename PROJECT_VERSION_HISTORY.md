...
| 第四版补丁 | `stable/v4.0.1` | `v4.0.1` | 在前端登录页展示老师查看用的普通用户和管理员两组测试账号密码，并同步说明文档、前端包版本与 Docker 镜像版本标签。 |
| 第五版 | `stable/v5` | `v5.0.0` | 完善 README 项目说明和全页面截图，统一前端时间显示为中文年月日时间，并同步说明文档、前端包版本与 Docker 镜像版本标签。 |
...
- 前端登录页新增“演示登录账号”区域，只展示普通用户和管理员两组平台登录测试账号密码。
- 说明文档补充老师查看账号说明，并明确不展示删除审计记录密码或解除封禁密码。
- 前端包版本和 Dockerfile 镜像版本标签同步到 `4.0.1`。

### v5.0.0

- 顶部当前时间和管理员审计时间统一显示为 `xxxx年xx月xx日 HH:mm:ss`。
- README 扩写项目定位、系统能力、技术架构、数据口径、运行方式和页面说明。
- README 按登录页、首页、全国态势、城市机会、薪资评估、技能趋势、职业建议、数据概览、模型评估、洞察报告和平台状态顺序放置页面截图。
- 前端包版本、Dockerfile 和 Docker Compose 镜像版本标签同步到 `5.0.0`。
...
```powershell
git fetch origin
git switch -c fix/from-v5 stable/v5
```
...
```powershell
git fetch origin
git switch -c fix/from-v4.0.1 stable/v4.0.1
```
