...
`v5.0.0` 起，后端继续支持通过 OpenRouter 生成 AI 职业推荐和薪资解释，并保留管理员登录审计、删除密码防爆破、临时封禁 IP 和解除封禁能力。本版本同步 Dockerfile 与 Docker Compose 镜像版本标签为 `5.0.0`，在前端登录页展示老师查看用的普通用户 `用户 / 123456` 和管理员 `admin / admin123` 两组测试账号，并将页面时间统一显示为 `xxxx年xx月xx日 HH:mm:ss`。AI 密钥、管理员安全 pepper 和密码哈希都只应通过后端环境变量传入，不能写入前端代码或提交真实值到 Git。
