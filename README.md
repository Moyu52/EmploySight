...
当前版本：`v4.0.1`。本版本在 `v4.0.0` 管理员安全能力基础上，将老师查看用的两组登录测试账号密码展示在前端登录页：普通用户 `用户 / 123456`，管理员 `admin / admin123`。
...
- 登录入口：默认先进入平台访问页，登录页展示普通用户 `用户 / 123456` 和管理员 `admin / admin123` 两组测试账号；普通账号进入分析平台，`admin` 账号进入平台状态与登录 IP 审计。
...
登录页只展示普通用户和管理员两组平台登录测试账号，不展示删除审计记录密码或解除封禁密码。

Docker Compose 部署时，把同样的 `AI_API_KEY` 和管理员安全哈希填到项目根目录 `.env`。未配置 AI 密钥时系统会继续使用本地规则推荐；未配置管理员安全哈希时，管理员删除和解除封禁操作会被后端拒绝。
